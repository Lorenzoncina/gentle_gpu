...
namespace kaldi{
  void ConfigFeatureInfo(OnlineNnet2FeaturePipelineInfo& info,
    std::string ivector_model_dir) {
      ...
      // XXX: Where do these come from? ivector_extractor.conf?
  ...
  void ConfigDecoding(LatticeFasterDecoderConfig& config){
  ...
  void ConfigEndpoint(OnlineEndpointConfig& config) {
...
*/
    ...
    while(true) {
      ...
      if(strcmp(cmd,"stop\n") == 0) {
        ...
        new (&decoder) SingleUtteranceNnet3Decoder(decoder_opts,
          trans_model,
          decodable_info,
          *decode_fst,
          &feature_pipeline);
        ...
        //myfile << "Advanced decoding" << std::endl ;
        //myfile << "OK";
        //myfile.flush();
        //fprintf(stdout, "ok\n");
        ...
        std::cout << "done with words" << std::endl;
      }
      else {
